...
class ChannelCommand extends Commando.Command {
    constructor(client) {
        super(client, {
            ...
            group: 'util',
            memberName: 'channel',
            description: 'Shows or sets the config channel.',
            format: '[#channel/"none"]',
            details: oneLine`
				If no channel is provided, the current config channel will be shown.
				If the channel is "none", the prefix will be removed entirely, preventing new configs to be added.
				Only administrators may change the channel.
			`,
            examples: ['channel', 'channel #config', 'channel none'],
            guildOnly: true,
            args: [
                {
                    key: 'channel',
                    prompt: 'What would you like to set the config channel to?',
                    type: 'string',
                    max: 21,
                    default: '',
    ...
    async run(msg, args) {
        ...
        if(!args.channel) {
            ...
            let reply;
            if (channel) reply = `The config channel is <#${channel}>.`;
            else reply = 'There is no config channel defined yet.';
